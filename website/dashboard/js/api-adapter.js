window.Adapter = (() => {
  ...
  async function listStudentsAdmin(filters={}){
    ...
    const rows=asArray(await req('/admin/students?'+q));
    const [departmentsRaw,semestersRaw]=await Promise.all([req('/departments'),req('/semesters')]);
    const departments=asArray(departmentsRaw), semesters=asArray(semestersRaw);
  ...
  async function listScheduleAdmin(filters={}){const q=new URLSearchParams({limit:'100'});if(filters.departmentId)q.set('departmentId',filters.departmentId);if(filters.semester)q.set('semesterId',filters.semester);const [rowsRaw,depsRaw,semsRaw,subsRaw]=await Promise.all([req('/admin/schedule?'+q),req('/departments'),req('/semesters'),req('/admin/subjects?limit=100')]);const rows=asArray(rowsRaw),deps=asArray(depsRaw),sems=asArray(semsRaw),subs=asArray(subsRaw);return {entries:rows.map(r=>({id:r.id,departmentId:r.department_id,semester:r.semester_id,dayOfWeek:r.day_of_week,startTime:r.start_time,endTime:r.end_time,subjectId:r.subject_id,subjectName:(subs.find(x=>x.id===r.subject_id)||{}).name_ar||'',room:r.room||'',instructor:r.lecturer||'',active:!!r.active})),departments:deps.map(d=>({id:d.id,name:{ar:d.name_ar,en:d.name_en}})),semesters:sems,subjects:subs};}
  ...
  async function syncDriveMaterials(){return req('/admin/drive/sync?nocache=1',{method:'POST'});}
  async function listMaterialsAdmin(){const [rowsRaw,subjectsRaw,depsRaw,semsRaw]=await Promise.all([req('/admin/materials?limit=100'),req('/admin/subjects?limit=100'),req('/departments'),req('/semesters')]);const rows=asArray(rowsRaw),subjects=asArray(subjectsRaw),deps=asArray(depsRaw),sems=asArray(semsRaw);const tree=deps.filter(d=>d&&d.id).map(d=>({id:d.id,name:d.name_ar||d.name_en||d.code||d.id,semesters:sems.filter(se=>se&&se.id).map(se=>({id:se.id,name:se.name_ar||se.name_en||(`الفصل ${se.number??se.id}`),subjects:subjects.filter(s=>s.department_id===d.id&&s.semester_id===se.id).map(s=>({id:s.id,name:s.name_ar||s.name_en||s.code||s.id,files:rows.filter(m=>m.subject_id===s.id).map(m=>({id:m.id,name:m.title||m.name||'ملف PDF',link:m.drive_web_view_url||m.drive_url||'#'}))}))}))}));return {tree};}

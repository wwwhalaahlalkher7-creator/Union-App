...
class InteractionsRepository {
  ...
  Future<List<CommentItem>> comments(String type,String id,{int offset=0}) async {
    final c = await AuthenticatedClient.create();
    try {
      final j=await c.getJson('/api/v1/content/$type/$id/comments',query:{'limit':'20','offset':'$offset'});
      final rows=j['data'];
      if(rows is! List)return const [];
      return rows.whereType<Map>().map((x)=>CommentItem.fromJson(Map<String,dynamic>.from(x))).toList();
    } finally {
      c.dispose();
    }
  }
